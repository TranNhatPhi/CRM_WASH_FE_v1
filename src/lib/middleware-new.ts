...
type AsyncHandler = (request: NextRequest, context?: any) => Promise<NextResponse>;
...
export function withValidation(handler: AsyncHandler): AsyncHandler {
    return async (request: NextRequest, context?: any) => {
        try {
            return await handler(request, context);
        } catch (error) {
...
// Logging middleware
export function withLogging(handler: AsyncHandler): AsyncHandler {
    return async (request: NextRequest, context?: any) => {
...
export function withRateLimit(maxRequests: number = 100, windowMs: number = 60000) {
    return function (handler: AsyncHandler): AsyncHandler {
        return async (request: NextRequest, context?: any) => {
...
// CORS middleware
export function withCORS(handler: AsyncHandler): AsyncHandler {
    return async (request: NextRequest, context?: any) => {
        const response = await handler(request, context);
...
export function compose(...middlewares: ((handler: AsyncHandler) => AsyncHandler)[]) {
    return function (handler: AsyncHandler): AsyncHandler {

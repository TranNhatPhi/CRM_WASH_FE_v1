...
export default function POSPage() {
  ...
  const checkVehicleRego = async () => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/vehicles/check?rego=${encodeURIComponent(carRego)}`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok && result.statusCode === 200) {
        if (result.data.found) {
          ...
          setIsVipCustomer(result.data.customer.tags && result.data.customer.tags.includes('VIP'));
        } else {
          // Customer doesn't exist - show full form modal
        ...
        throw new Error(result.message || 'Failed to check vehicle registration');
      }
    } catch (error) {
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Lỗi kết nối',
        text: 'Không thể kiểm tra biển số. Vui lòng thử lại.',
    ...
    try {
      ...
      const customerVehicleData = {
        ...
        customer: {
          ...
          email: customerEmail.trim() || "nguyenvan@example.com",
          address: customerAddress.trim() || "123 Street, District, HCMC",
          customer_notes: customerNote.trim() || "VIP customer"
        }
      };
      ...
      if (response.ok && result.statusCode === 201) {
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Thành công!',
          text: 'Tạo khách hàng và phương tiện thành công!',
          timer: 5000,
      ...
      await Swal.fire({
        ...
        text: error instanceof Error ? error.message : 'Không thể tạo khách hàng',
        confirmButtonColor: '#ef4444',
        background: isDarkMode ? '#1f2937' : '#ffffff',
        color: isDarkMode ? '#f3f4f6' : '#1f2937',
      });
